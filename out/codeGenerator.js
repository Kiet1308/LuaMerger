...
class CodeGenerator {
    ...
    generate(graph, entryModule, clientScripts = []) {
     ...
     */
    collectFolderPaths(moduleNames) {
     ...
     */
    generateFolderDeclarations(folders) {
        if (folders.length === 0)
            return '';
    ...
    generateModuleWrapper(module, allModuleNames) {
    ...
    generateClientScripts(clientScripts) {
        const lines = [
            '',
